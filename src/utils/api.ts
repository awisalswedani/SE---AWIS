...
export async function getSellerProducts(params: Record<string, string> = {}) {
  ...
  const res = await fetch(`${BASE_URL}/products/latest?${query}`, {
    headers: {
      'Content-Type': 'application/json',
    },
    next: { revalidate: 60 },
  ...
  if (data.products && Array.isArray(data.products)) {
    ...
    data.products = data.products.filter((product: any) => {
      const sellerId = product.seller_id || product.sellerId || product.user_id || product.userId;
      const shopId = product.shop_id || product.shopId;
      
      const nestedSellerId = product.seller?.id || product.seller?.seller_id;
      
      console.log(`Product ${product.id}: seller_id=${product.seller_id}, sellerId=${product.sellerId}, user_id=${product.user_id}, shop_id=${product.shop_id}, nested=${nestedSellerId}`);
      
      return sellerId?.toString() === SELLER_ID || 
             shopId?.toString() === SELLER_ID ||
             nestedSellerId?.toString() === SELLER_ID;
    });
    
    console.log(`Filtered ${originalCount} products to ${data.products.length} for seller ${SELLER_ID}`);
  }
...
export async function addToCart(productId: number, quantity: number = 1, variant?: any) {
  const guestId = await getOrCreateGuestId();
  
  const data: any = {
    id: productId, // Use 'id' instead of 'product_id' to match Flutter app
    guest_id: guestId,
    quantity: quantity,
    buy_now: 0,
  };
  ...
  if (variant) {
    ...
    if (variant.variant_key) {
      data.variant_key = variant.variant_key;
      data.digital_variation_price = variant.digital_variant_price;
    }
  }
