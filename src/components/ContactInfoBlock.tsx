...
export default function ContactInfoBlock({ className = '', onlySocial = false }: { className?: string, onlySocial?: boolean }) {
  return (
    <div className={`flex flex-col items-center ${onlySocial ? '' : 'space-y-1'} ${className}`}>
      {!onlySocial && (
        <>
          <span className="text-lg font-semibold" dir="ltr">{WEB_CONSTANTS.phone}</span>
          <span className="text-base text-gray-300" dir="ltr">{WEB_CONSTANTS.email}</span>
        </>
      )}
